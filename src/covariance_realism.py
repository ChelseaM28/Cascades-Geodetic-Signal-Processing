...
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -
# REFACTORED CODE (My notes prior to refactor can still be found at bottom of script.)
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -


class Station:
    def __init__(self, station_direction):
        self.station_direction = station_direction #formatted 'p349'
        self.station_id = station_direction[:5] #pXXX
        self.residuals = residuals[self.station_direction]

        # Used to parametrically derive the Cs for GLS and Monte Carlo error series
        self.n = station_lengths[self.station_id]
        self.slope = None
        self.intercept = None 
        self.var_white = None
        self.var_colored = None
        self.J = None 
        self.H = None 
        self.C = None 

        #Holds the synthetic series and true velocities generated by Monte Carlo
        self.series_list = [] 
        self.true_velocities = [] 
        self.L = None

        #Holds the metrics derived after fitting OLS and GLS to each synthetic series.
        self.p = 6 #number of parameters in my model: a + b + c + d + e + f
        self.GLS_metrics = [] 
        self.OLS_metrics = [] 

        #Results from the distribution comparison
        self.ols_results = None
        self.gls_results = None
        
    def fit_psd_line(self):
        #This is the process that finds the alphas (k values)
        freqs, power = periodogram(self.residuals, fs=365.25)
        freq_cutoff = 5
        ...
        self.slope, self.intercept = np.polyfit(log_f, log_p, 1) 
        return self.slope, self.intercept

    def create_general_power_law_cov_matrix(self):
        h = [1] 
    ...
    def find_characterized_var(self):
        white_freq = 10**(0.7)
        ...
        white_log_power = self.slope * np.log10(white_freq) + self.intercept
        self.var_white = 10 ** white_log_power

        colored_log_power = self.slope * np.log10(colored_freq) + self.intercept
        self.var_colored = 10 ** colored_log_power

        return self.var_white, self.var_colored

    def create_parametric_C(self):
        #set k, H, and J
        I = np.eye(self.n)
        ...
        self.C = self.var_colored*self.J + self.var_white*I
        return self.C
    ...
    def generate_monte_carlo_series(self):
        ...
        for simulation in range(N):
            v= np.random.normal(loc=0.0, scale=1.0, size=self.n)
            w = self.L@v #The noise model applied to w. See slide deck for the math.
    ...
    def fit_LS_models(self):
        
        #OLS SECTION
        X = X_matrices[self.station_id]
        ...
        self.OLS_metrics = OLS_cov_realism_metric #z^2 is the squared Mahalanobis Distance metric from Poore (at DoF = 1)
    ...
    def main(self):
        self.fit_psd_line()
        self.create_general_power_law_cov_matrix()
        self.find_characterized_var()
        self.create_parametric_C()
        self.generate_monte_carlo_series()
        self.fit_LS_models()
        return self.OLS_metrics, self.GLS_metrics

def plot_histograms(model, realism_metrics):
    plt.hist(realism_metrics, bins= 100, edgecolor='black')
    plt.title(f"Histogram of {model} Realism Metrics")
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"/workspaces/GNSS/data/graphics/Uncertainty Quant Round 2/{model}_Histogram.png", dpi=120)
    print(f"Saved {model}_Histogram")
...
def main():
    ...
    for station_direction in station_directions:
        station_dir_object = Station(str(station_direction))
        GLS_metrics, OLS_metrics = station_dir_object.main()
...
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -
# Describe OLS and GLS equations.
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -
...
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -
# Use Monte Carlo to Generate N Synthetic Ground Station Series
# * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * -
